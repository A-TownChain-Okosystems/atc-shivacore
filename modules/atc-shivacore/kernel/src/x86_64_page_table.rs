...
pub enum PageTableError { InvalidPid, OutOfFrames, InvalidRoot, Mapping(UserMappingError), NotMapped, ProcessExists, ProcessMissing, RootOwnershipConflict }
impl From<UserMappingError> for PageTableError { fn from(value: UserMappingError) -> Self { Self::Mapping(value) } }

pub struct ProcessPageTable { pid: Pid, root_frame: PhysFrame, physical_memory_offset: VirtAddr }
impl ProcessPageTable {
    ...
    pub unsafe fn map_user_page(&mut self, page: Page<Size4KiB>, frame: PhysFrame, flags: PageFlags, frame_allocator: &mut BootInfoFrameAllocator) -> Result<(), PageTableError> { let mut mapper = self.mapper(); map_user_page(&mut mapper, page, frame, flags, frame_allocator).map_err(Into::into) }
    pub unsafe fn unmap_user_page(&mut self, page: Page<Size4KiB>) -> Result<PhysFrame, PageTableError> { let mut mapper = self.mapper(); let (frame, flush) = mapper.unmap(page).map_err(|_| PageTableError::NotMapped)?; flush.flush(); Ok(frame) }
...
/// a physical L4 frame may belong to exactly one live PID and cannot be
/// registered twice. It does not imply that the frame can be returned to the
/// boot allocator; that allocator currently has no reclamation API.
