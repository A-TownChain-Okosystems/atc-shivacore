...
pub enum ContextSwitchError { ProcessMissing, InvalidState, AddressSpace(PageTableError), ContextRootMismatch, KernelStackMissing, KernelStackActivation, ContextMissing, InvalidTimerContext }
...
impl ProcessScheduler {
    ...
    pub unsafe fn activate_context<A: KernelStackActivator>(&mut self, context: &ProcessExecutionContext, stacks: &KernelStackManager, activator: &mut A) -> Result<!, ContextSwitchError> {
        ...
        self.register_context(context)?; self.bind_kernel_stack(pid, stacks, activator)?; self.address_spaces.switch_to(pid)?; self.ready.retain(|queued| *queued != pid); if let Some(old) = self.current { self.states.insert(old, RunState::Ready); self.ready.push_back(old); } self.states.insert(pid, RunState::Running); self.current = Some(pid); switch_to_context(context.context_stack())
    }
    ...
    pub fn unregister_process(&mut self, pid: Pid) -> Result<(), ContextSwitchError> { if self.current == Some(pid) { return Err(ContextSwitchError::InvalidState); } self.address_spaces.destroy(pid)?; self.ready.retain(|queued| *queued != pid); self.states.remove(&pid); self.contexts.remove(&pid); Ok(()) }
}

#[cfg(test)]
mod tests { use super::*; #[test] fn starts_without_current_process() { let scheduler = ProcessScheduler::new(ProcessAddressSpaceManager::new()); assert_eq!(scheduler.current(), None); } #[test] fn yield_without_current_is_rejected_without_state_mutation() { let mut scheduler = ProcessScheduler::new(ProcessAddressSpaceManager::new()); assert_eq!(scheduler.yield_current(), Err(ContextSwitchError::InvalidState)); assert_eq!(scheduler.current(), None); } #[test] fn context_root_mismatch_is_distinct_from_process_missing() { assert_ne!(ContextSwitchError::ContextRootMismatch, ContextSwitchError::ProcessMissing); } #[test] fn context_missing_is_distinct_from_invalid_timer_context() { assert_ne!(ContextSwitchError::ContextMissing, ContextSwitchError::InvalidTimerContext); } }
