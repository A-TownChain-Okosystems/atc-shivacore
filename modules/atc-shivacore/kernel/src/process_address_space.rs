...
impl ProcessAddressSpaces {
    pub const fn new() -> Self { Self { spaces: BTreeMap::new() } }
    ...
    pub fn check_access(&self, pid: Pid, addr: u64, len: u64, required: PageFlags) -> Result<(), AddressSpaceError> {
        self.spaces.get(&pid).ok_or(AddressSpaceError::ProcessMissing)?.check_access(addr, len, required).map_err(Into::into)
    }

    pub fn mappings(&self, pid: Pid) -> Result<impl Iterator<Item = &Mapping>, AddressSpaceError> {
        self.spaces.get(&pid).map(AddressSpace::mappings).ok_or(AddressSpaceError::ProcessMissing)
    }
}
...
mod tests {
    ...
    fn mapping(start: u64, size: u64) -> Mapping {
        Mapping { start, size, flags: PageFlags::READ.union(PageFlags::USER) }
    }
    ...
    fn overlapping_mapping_is_rejected() {
        ...
        let pid = Pid(7);
        spaces.create(pid).unwrap();
        spaces.map(pid, mapping(0x1_0000_0000, 8192)).unwrap();
        assert!(spaces.map(pid, mapping(0x1_0000_1000, 4096)).is_err());
    }
}
