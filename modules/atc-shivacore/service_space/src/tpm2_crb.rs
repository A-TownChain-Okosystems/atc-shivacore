...
impl<M: MmioAccess> TpmCrbTransport<M> {
    /// Execute one TPM command through CRB with bounded polling.
    pub fn execute(&mut self, command: &[u8], response: &mut [u8], poll_limit: usize) -> Result<usize, CrbTransactionError> {
        ...
        let rsp_addr = self.register_address(crb::RSP_ADDR)?;
        ...
        let response_address = self.mmio.read_u32(rsp_addr)? as u64;
