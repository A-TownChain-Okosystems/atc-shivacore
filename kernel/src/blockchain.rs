...
pub fn add_block(&self,block:Block)->Result<(),PipelineError>{let mut b=self.blocks.lock();let mut bh=self.by_hash.lock();let mut h=self.current_height.lock();if block.height!=*h+1{return Err(PipelineError::InvalidHeight);}if b.contains_key(&block.height){return Err(PipelineError::BlockExists);}if bh.contains_key(&block.id){return Err(PipelineError::DuplicateBlock);}bh.insert(block.id,block.height);b.insert(block.height,block);*h=block.height;Ok(())}
...
pub fn create_genesis(&self,timestamp:u64)->Result<Block,PipelineError>{let pe=self.consensus.poh().tick(timestamp);let g=Block::new(0,[0u8;32],self.our_did.clone(),timestamp,pe.hash,vec![],[0u8;64]);self.chain.add_genesis(g.clone())?;*self.block_height.lock()=0;Ok(g)}
